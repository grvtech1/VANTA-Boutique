...
| 5 | **Build, scan and push all twelve images ourselves** (`docker.io/grvp1/<svc>:<git-sha>`) | One registry, one provenance, one scan gate for everything that runs; no runtime dependency on an upstream or cloud-provider registry | Twelve build jobs per pipeline run (cached per service) |
